...
1. 前端如何实现截图 （canvas svg）
2. QPS 达到峰值时如何处理 （避免达到峰值；达到峰值后怎么办）
3. js 超过 Number 最大值的数怎么处理 （BigInt 字符串）
4. 使用同一个链接，如何实现 PC 打开是 web 应用，手机打开是 H5 应用 （响应式布局、 meta、 user-agent 判断跳转）
5. 设计一套全站请求耗时的工具 （拦截器 PerformanceObserver?）
6. 大文件上传 (File.slice、断点续传)
7. H5 移动端适配的问题
8. 站点一键换肤 （js 修改 class、css变量、属性选择器、alternate stylesheet 可替代样式表）
9. 实现网页加载进度条
10. 图片懒加载
11. cookie 的构成部分
12. 扫码登录
...
15. 水印
16. DDD
17. 变量一直挂在 window 上的风险
18. seo
19. 小程序为什么有两个线程
20. web 应用中如何对静态资源加载失败的场景做降级处理 （onerror fallback 动态加载 script cdn）
